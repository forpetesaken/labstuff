#!/usr/bin/env python3
"""
Chat follow-up conservation analysis for CTCF.
...
IN_FASTA = Path(
    r"C:\Users\Nat\Downloads\AIDEN Lab\Code\alignment_work\alignments_out\CTCF\top_hit_CTCF_Xenopus_laevis.fas"
)
OUT_DIR = Path(
    r"C:\Users\Nat\Downloads\AIDEN Lab\Code\alignment_work\alignments_out\CTCF\05\chat_followup"
)
...
def main() -> None:
    ...
    if "INVERTEBRATES" not in headers:
        raise ValueError("Missing INVERTEBRATES marker sequence")

    split_idx = headers.index("INVERTEBRATES")
    vertebrate_records = records[:split_idx]
    invertebrate_records = records[split_idx + 1 :]
    ...
    axes[0].plot(x_h, moving_average(y_v, 25), color="#006d2c", lw=1.7, alpha=0.9)
    axes[0].plot(x_h, moving_average(y_i, 25), color="#a50f15", lw=1.7, alpha=0.9)
    ...
    axes[1].plot(x_h, moving_average(y_d, 25), color="#08519c", lw=1.7, label="Smoothed delta")
