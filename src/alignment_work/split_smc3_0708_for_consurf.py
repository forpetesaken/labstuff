...
IN_FASTA = Path(r"C:\Users\Nat\Downloads\AIDEN Lab\Code\alignment_work\ConSurf\output\SMC3\smc3_0708.fas")
OUT_DIR = Path(r"C:\Users\Nat\Downloads\AIDEN Lab\Code\alignment_work\ConSurf\input_splits\SMC3")
...
def main():
    ...
    full_s = sanitize(records, "ALL", "human", "Human_SMC3")
    write_fasta(OUT_DIR / "smc3_full_all.fasta", full_s)

    vert_s = sanitize(vertebrates, "VERT", "human", "Human_SMC3")
    write_fasta(OUT_DIR / "smc3_vertebrates_0708_sanitized.fasta", vert_s)

    invert_s = sanitize(invertebrates, "INVERT", "ciona", "Ciona_intestinalis")
    write_fasta(OUT_DIR / "smc3_invertebrates_0708_sanitized.fasta", invert_s)
    ...
    summary = [
        ...
        "Vertebrate query: Human_SMC3",
        "Invertebrate query: Ciona_intestinalis",
    ]
    (OUT_DIR / "smc3_0708_split_summary.txt").write_text("\n".join(summary) + "\n", encoding="utf-8")
    print("Done")
