...
IN_FASTA = Path(r"C:\Users\Nat\Downloads\AIDEN Lab\Code\alignment_work\ConSurf\output\PDS5A\psd5A_0708.fas")
OUT_DIR = Path(r"C:\Users\Nat\Downloads\AIDEN Lab\Code\alignment_work\ConSurf\input_splits\PDS5A")
...
def find_boundary_index(records):
    for idx, (h, _) in enumerate(records):
        ...
        if any(marker.lower() in hl for marker in BOUNDARY_MARKERS):
            return idx
    raise ValueError("Could not detect PDS5A invertebrate boundary")
...
def main():
    ...
    full_s = sanitize(records, "ALL", "human", "Human_PDS5A")
    vert_s = sanitize(vertebrates, "VERT", "human", "Human_PDS5A")
    invert_s = sanitize(invertebrates, "INVERT", "ciona", "Ciona_intestinalis")

    write_fasta(OUT_DIR / "pds5a_full_0708_sanitized.fasta", full_s)
    write_fasta(OUT_DIR / "pds5a_vertebrates_sanitized.fasta", vert_s)
    write_fasta(OUT_DIR / "pds5a_invertebrates_sanitized.fasta", invert_s)
    ...
    (OUT_DIR / "pds5a_split_summary.txt").write_text("\n".join(summary) + "\n", encoding="utf-8")
    print("Done")
